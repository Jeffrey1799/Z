...
【模式概览】
  -pull                一键同步模式（仅父仓库环境）。拉取更新所有/指定子仓库至最新。
  -add (别名: -build)  接入子仓库模式。读取子仓库 origin 与分支，在父仓库中注册。
  -rm (别名: -Remove)  解绑子仓库模式。仅解除父仓库跟踪指针，【绝对保留】本地物理文件。
  -help (别名: -h, -?) 显示本帮助说明。
...
--------------------------------------------------------------------------------
1. 一键同步子仓库：
   .\zgit.ps1 -pull                             # 同步所有子仓库至远程指定分支最新
   .\zgit.ps1 -pull -Submodule firmware         # 仅同步指定子仓库
   .\zgit.ps1 -pull -SkipParentPull             # 跳过父仓库 git pull，只更新子仓库

2. 注册接入子仓库：
   .\zgit.ps1 -add C:\work\host-app             # 将外部子仓库接入父仓库注册
   .\zgit.ps1 -add host-app                     # 将父仓库内已有的子仓库原地注册并推送至远程
   .\zgit.ps1 -add C:\work\host-app -b dev      # 接入并指定跟踪 dev 分支

3. 解绑子仓库指针：
   .\zgit.ps1 -rm -Submodule algorithm          # 解绑指定子仓库指针（保留本地磁盘代码）
   cd algorithm; ..\zgit.ps1 -rm                # 在子仓库目录执行，自动识别当前目录解绑

--------------------------------------------------------------------------------
【场景二：在只有子仓库目录的环境下（分发场景）】
--------------------------------------------------------------------------------
1. 注册接入父仓库（无需本地克隆父仓库）：
   .\zgit.ps1 -add                              # 自动识别当前子仓库，注册进默认父仓库
   .\zgit.ps1 -add -ParentUrl <父仓库URL>       # 注册接入指定的其他父仓库

2. 解绑远程父仓库指针（无需本地克隆父仓库）：
   .\zgit.ps1 -rm                               # 自动识别当前子仓库，从默认父仓库中解绑
   .\zgit.ps1 -rm -ParentUrl <父仓库URL>        # 从指定的其他父仓库中解绑

3. 日常同步说明：
   单子仓库环境下不可使用 -pull。日常更新代码请直接运行：git pull --ff-only

--------------------------------------------------------------------------------
【通用与扩展参数】
--------------------------------------------------------------------------------
  -Submodule <名称/路径> 指定要操作的目标子仓库（支持逗号分隔多个）。
  -DryRun               预览模式。仅显示将要执行的操作，不修改任何文件与引用。
  -NoPush               配合 -add 或 -rm 使用，提交本地变更后跳过 git push 推送。
  -ParentUrl <URL>      配合 -add 或 -rm 使用，指定/覆盖远程父仓库地址。
  -AddBranch (别名: -b) 配合 -add 使用，指定跟踪分支（默认读取源仓库当前分支）。
  -AddName (别名: -n)   配合 -add 使用，指定子模块注册名称（默认取目录名）。
  -AddModulePath        配合 -add 使用，指定保存在父仓库中的相对路径。
